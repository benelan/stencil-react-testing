...
const defineCustomElementImports =
  /import { defineCustomElement as defineMy.*(\r\n|\r|\n)/gm;
...
const reactWrapperExports =
  /createReactComponent<(.*)>.*\((['|\w|-]*)(.*)(defineMy\w*)\)/g;
...
const patchedReactWrapperExports =
  "createReactComponent<$1>($2$3autoDefine($2))";

// The autoDefine import is placed below this line
const reactLibImport =
...
(async () => {
  try {
    ...
    if (contents.includes(autoDefineImport)) {
      console.log("skipping patch, components.ts is already patched");
      return;
    }
